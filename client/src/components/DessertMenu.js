...
const DessertMenu = ({ cart, addToCart }) => {
  const navigate = useNavigate();
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const categoryId = 5; // Example categoryId. Replace with a dynamic value if needed.

  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`https://swiftel.co.ke/api/categories/5/products`);
